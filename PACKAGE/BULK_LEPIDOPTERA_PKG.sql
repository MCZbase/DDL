  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE "BULK_LEPIDOPTERA_PKG" as
	PROCEDURE check_and_load;
	PROCEDURE bulkloader_check;
END;
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "BULK_LEPIDOPTERA_PKG" as
...
begin
	...
	update bulkloader_lepidoptera set loaded = error_msg where collection_object_id = collobjid;
EXCEPTION
	when others then
...
is
 thisError varchar2(4000);
  BEGIN
	FOR rec IN (SELECT collection_object_id FROM bulkloader_lepidoptera where loaded is null) LOOP
		SELECT bulk_lep_check_one(rec.collection_object_id) INTO thisError FROM dual;
		if thisError is not null then
			if length(thisError) > 224 then
				thisError := substr(thisError,1,200) || ' {snip...}';
			end if;
			rollback;
			update bulkloader_lepidoptera set loaded = thisError where collection_object_id = rec.collection_object_id;
		end if;
		commit;
	END LOOP;
END;
...
is
	...
	b_locality_id  bulkloader_lepidoptera. locality_id%TYPE;
	error_msg varchar2(4000);  
...
BEGIN
	...
	for i IN 1 .. 10 LOOP -- number of attributes
		execute immediate 'select count(*) from bulkloader_lepidoptera where ATTRIBUTE_' || i || ' is not null and 
		...
		if num = 1 then -- there's an attribute - insert it
			...
			execute immediate 'select ATTRIBUTE_DETERMINER_' || i || ' from bulkloader_lepidoptera where collection_object_id = ' || 
			...
			execute immediate 'select ATTRIBUTE_' || i || 
				...
				' from bulkloader_lepidoptera where collection_object_id = ' || collobjid into
				ATTRIBUTE,
				ATTRIBUTE_VALUE,
				ATTRIBUTE_UNITS,
				ATTRIBUTE_REMARKS,
...
r_preserve_method specimen_part.preserve_method%TYPE;
...
c_cnt number;
new_container_id container.container_id%type;
new_container_type container.container_type%type;
target_parent_container_id container.parent_container_id%type;
...
BEGIN
	...
	select institution_acronym into institution_acronym from bulkloader_lepidoptera where collection_object_id = collobjid;
	--dbms_output.put_line ('got institution_acronym...');
	for i IN 1 .. 12 LOOP -- number of parts
	    --dbms_output.put_line('on part loop ' || i);
	    
		execute immediate 'select count(*) from bulkloader_lepidoptera where PART_NAME_' || i || ' is not null 
			and collection_object_id = ' || collobjid into num;
		if num = 1 then -- there's a part - insert it
				----dbms_output.put_line ('inserting a part...');
        --  only part barcode 1 is populated, applies to all parts
        --  only part container label 1 is populated, applies to all parts
        ...
        r_lotcountmodifier,
			...
			execute immediate 'select institution_acronym || '' '' || collection_cde || '' ' || l_cat_num || ' ''  || 
				part_name_' || i ||  ' from bulkloader_lepidoptera where collection_object_id = ' || 
      ...
      --  Trigger MAKE_PART_COLL_OBJ_CONT creates a container of type collection object for this part.
          ...
          select count(*) into c_cnt from container where barcode = r_barcode;
          case r_partname || r_preserve_method
            when 'pinned' then new_container_type := 'pin';
            when 'partial animalpinned' then new_container_type := 'pin';
            when 'genitaliapinned' then new_container_type := 'pin';  
            when 'genitaliacapsule' then new_container_type := 'pin';
            when 'whole animal' then new_container_type := 'pin';            
            when 'whole animalpinned' then new_container_type := 'pin';      
            ...
            when 'pupal exuviapointed' then new_container_type := 'pin'; 
            ...
            when 'partial animal: abdomencarded' then new_container_type := 'pin';
            when 'partial animal: bodyenvelope' then new_container_type := 'envelope';
            when 'partial animal: bodypointed' then new_container_type := 'pin';
            when 'partial animal: bodypinned' then new_container_type := 'pin';
            when 'partial animal: bodycarded' then new_container_type := 'pin';
            when 'partial animal: legsenvelope' then new_container_type := 'envelope';
            when 'partial animal: legspointed' then new_container_type := 'pin';
            when 'partial animal: legspinned' then new_container_type := 'pin';
            when 'partial animal: legscarded' then new_container_type := 'pin';
            when 'partial animal: wingsenvelope' then new_container_type := 'envelope';
            when 'partial animal: wingspointed' then new_container_type := 'pin';
            when 'partial animal: wingspinned' then new_container_type := 'pin';  
            when 'partial animal: wingscarded' then new_container_type := 'pin'; 
          end case;
          If c_cnt = 0 then
            -- r_label := r_barcode;
            
            --  Find the parent container to place the new container into 
            -- SELECT 0 into target_parent_container_id FROM dual;
            select mczbase.get_parentfor_datashot(mczbase.find_familyforgenus(genus), locationincollection) into target_parent_container_id 
                from  specimen@lepidoptera 
                where specimenid = collobjid;
            
            SELECT sq_container_id.nextval into new_container_id FROM dual;
            -- If one doesn't exist, add a parent container (usually of type pin for the container of type collection object).
            Insert into container(container_id, institution_acronym, container_type, label, parent_container_id, locked_position, barcode, parent_install_date, container_remarks)
            values(new_container_id, institution_acronym, new_container_type, r_barcode, target_parent_container_id, 0, r_barcode, SYSDATE, r_label);
          end if;
          -- get the current part container id
        ...
        -- retain the label generated by the trigger for the collection object container  
    ...
    --bulk part attributes
          ...
          execute immediate 'select count(*) from bulkloader_lepidoptera where part_' || i || '_att_name_' || j || ' is not null and 
            part_' || i || '_att_val_' || j || ' is not null and collection_object_id = ' || collobjid into num;
            dbms_output.put_line ('num: ' || num);
          if num = 1 then -- there's an attribute - insert it
            select sq_attribute_id.nextval into ATTRIBUTE_ID from dual;
            --dbms_output.put_line ('ATTRIBUTE_ID: ' || ATTRIBUTE_ID);
            execute immediate 'select part_' || i || '_att_detby_' || j || ' from bulkloader_lepidoptera where collection_object_id = ' || 
              collobjid into ATTRIBUTE_DETERMINER;
              --dbms_output.put_line ('ATTRIBUTE_DETERMINER: ' || ATTRIBUTE_DETERMINER);
...
BEGIN
	for i IN 1 .. 5 LOOP -- number of other IDs		
		execute immediate 'select count(*) from bulkloader_lepidoptera where OTHER_ID_NUM_' || i || ' is not null 
			and collection_object_id = ' || collobjid into num;
		if num = 1 then -- there's an other ID number - insert it
			execute immediate 'select OTHER_ID_NUM_' || i || ', OTHER_ID_NUM_TYPE_' || i || ' from bulkloader_lepidoptera where 
...
rec bulkloader_lepidoptera%ROWTYPE;
...
BEGIN
		SELECT * into rec FROM bulkloader_lepidoptera where collection_object_id = collobjid;		
...
ATTRIBUTE_ID attributes.ATTRIBUTE_ID%TYPE;
BEGIN
	select * into aRec from bulkloader_lepidoptera where collection_object_id=collobjid;
	IF aRec.locality_id is null AND aRec.collecting_event_id IS NULL then -- it should always be
		...
		execute immediate 'select count(*) from bulkloader_lepidoptera where geology_attribute_' || i || ' is not null and 
...
BEGIN
	...
	if aRec.locality_id is null AND aRec.collecting_event_id IS NULL then -- otherwise, we already have what we need
     ...
     END IF; 
		...
		SELECT collecting_event_id INTO gCollEvntId from bulkloader_lepidoptera where collection_object_id = collobjid;
		IF nvl(length(gCollEvntId),0) = 0 THEN
		    ...
		    select locality_id into gLocalityId from bulkloader_lepidoptera where collection_object_id = collobjid;
		    select count(*)  into num from locality where locality_id=gLocalityId;
		    if num = 1 then
    			l_locality_id := gLocalityId;
    		else
    			error_msg := ' bulkloader_lepidoptera. locality_id does not resolve to a valid locality';
    			raise failed_validation;
    		end if;
    	ELSE
    	    ...
    	    if num != 1 then
    		    error_msg := ' bulkloader_lepidoptera. collecting_event_id does not resolve to a valid collecting_event';
    			raise failed_validation;
    		end if;
    	END IF;		
	END IF; -- locid is null check
...
l_taxon_name_id_2 := NULL;
   ...
   select count(distinct(accn.transaction_id)) into num from accn,trans,collection where 
    ...
    taxa_one := substr(rec.taxon_name,1,length(rec.taxon_name) - 5);
  elsif  substr(rec.taxon_name,length(rec.taxon_name) - 3) = ' sp. nov.' then
...
PROCEDURE b_bulk_this is
	CURSOR rec_cursor IS
		SELECT collection_object_id from bulkloader_lepidoptera where loaded is null and collection_object_id > 10;
	...
	l_loaded  bulkloader_lepidoptera. loaded%TYPE;
begin
	--b_bulk_disable;	
	FOR rec IN rec_cursor LOOP
		...
		if error_msg is null then
      insert into bulkloader_lepidoptera_map(specimenid, collection_object_id, collecting_event_id, locality_id)
      values(collobjid, l_collection_object_id, l_collecting_event_id, l_locality_id);
      update specimen@lepidoptera set flaginmczbase = 1 where specimenid=collobjid;
			delete from bulkloader_lepidoptera where collection_object_id = collobjid;
			--update bulkloader_lepidoptera set loaded = 'spiffification complete' where collection_object_id = collobjid;
		else
		...
		commit;
		/*
		select loaded into l_loaded from bulkloader_lepidoptera where collection_object_id = collobjid;
...
PROCEDURE check_and_load is
	num number;
begin
	-- relies on table proc_bll_status:
	-- create table proc_bll_status (status number(1));
	select count(*) into num from proc_bll_status;
	if num != 1 then
		delete from proc_bll_status;
		insert into proc_bll_status (status) values (0);
		commit;
	end if;
	select status into num from proc_bll_status;
	if num = 0 then
		-- lock this process
		update proc_bll_status set status=1;
		commit;
		--bulkloader_check;
		b_bulk_this;
		-- update status table to indicate loading attempt complete
		update proc_bll_status set status=0;
		commit;
	end if;
end;
---------------------------------------------------------------------------------------------------------------------------------------------
