...
for c in (select container_id,parent_container_id from container where parent_container_id=r.container_id) loop
if r.parent_container_id is null then
s:='update container set parent_container_id=null where container_id=' || c.container_id;
--dbms_output.put_line(s);
execute immediate(s);
else
s:='update container set parent_container_id=' || r.parent_container_id || ' where container_id='|| c.container_id;
--dbms_output.put_line(s);
execute immediate(s);
end if;
end loop;
