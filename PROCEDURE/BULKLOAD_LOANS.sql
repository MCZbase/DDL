  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "BULKLOAD_LOANS" as

cursor c1 is select * from CF_TEMP_LOAN where moved is null or moved = 'X';
numTRANSID number;
numCOLLECTIONID number;
cnt number;
err_num varchar(100);
...
if cnt = 0 then 
select collection_id into numCOLLECTIONID from collection where collection = c1_rec.collection;
...
INSERT INTO trans (
				...
				VALUES (
					...
					to_date(c1_rec.transaction_date, 'YYYY-MM-DD'),
					0,
					'loan',
...
update cf_temp_loan set transaction_id = numTRANSID, moved = 'Y', error = null where ID = numID;
commit;
    ...
    raise failed_validation;
      ...
      EXCEPTION
       ...
       ROLLBACK;
       update CF_TEMP_LOAN set error = err_msg, moved = 'X' where ID = numID;
       commit;
       ...
       UPDATE CF_TEMP_LOAN SET error = err_num || ': ' || err_msg, moved = 'X' WHERE ID=numID;
       COMMIT;
