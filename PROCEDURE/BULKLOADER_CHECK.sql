  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "BULKLOADER_CHECK" 
is
 thisError varchar2(255);
 numRecs NUMBER;
  ...
  BEGIN
	FOR rec IN (SELECT * FROM bulkloader where loaded is null) LOOP
		...
		IF (rec.relationship is not null) THEN
			...
			END IF;
			select  count(*) into numRecs from ctcoll_other_id_type WHERE
		...
		IF (rec.orig_lat_long_units is NOT null) THEN
			...
			END IF;
			IF (rec.max_error_distance is null OR isnumeric(rec.max_error_distance) = 0) THEN
      ...
      collectorRole := trim(collectorRole);
		...
		SELECT count(*) INTO numRecs FROM accn WHERE ACCN_NUMBER = rec.ACCN;
		if numRecs = 0 then
