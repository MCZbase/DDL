  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "BUILD_QUERY_DBMS_SQL_NEST" (
  resultID    IN varchar2,
  varUsername IN varchar2,
  searchJSON  IN CLOB
)
AS
  ...
  varPreparatorsJoin     varchar2(4000);
  ...
  --- Container subtree search support
  varContainersJoin             CLOB;
  varContainerAnchorConditional CLOB;
  hasContainerCriteria          number := 0;

  varJoinList varchar2(4000) := 'Cataloged Items';
  isCatNum boolean := false;
  varJoinTerm varchar(10);
  numOpenParens number := 0;
  numCloseParens number := 0;
  containLabel number := 0;
  ...
  varColumnName      varchar2(4000);  -- table_alias.column_name (or overridden)
  varSearchCategory  cf_spec_search_cols.search_category%TYPE;
  varDataType        cf_spec_search_cols.data_type%TYPE;

  --- capture base column name so we can override alias cleanly for container anchors
  varColumnBaseName  cf_spec_search_cols.column_name%TYPE;
  ...
  cursor c1 is select * from table (make_search_terms_nest(searchJSON));

BEGIN

  Select count(*) into oneOfUs
  from dba_role_privs
  where GRANTED_ROLE = 'COLDFUSION_USER'
    and grantee = upper(varUserName);
  ...
  varPreparatorsJoin :=
    '
     ---preparators view on collector table
     left outer join preparator on (cataloged_item.collection_object_id = preparator.collection_object_id)
     left outer join agent preparator_agent on (preparator.agent_id = preparator_agent.agent_id)
     left outer join person preparator_person on (preparator_agent.agent_id = preparator_person.person_id)
     left outer join agent_name preparator_agent_name on (preparator.agent_id=preparator_agent_name.agent_id)';
  ...
  for c1_rec in c1 loop
    ...
    -- Lookup the search field in cf_spec_search_cols.
    -- NOTE: we now also select column_name separately into varColumnBaseName.
    select
      table_name,
      table_alias,
      column_name,
    ...
    -- Add joins based on the search category.
    case varSearchCategory
      ...
      when 'Preparators' then
        if instr(varJoinList,varSearchCategory)=0 then
          varJOINS := varJOINS || varPreparatorsJoin;
          varJoinList := varJoinList || varSearchCategory;
        end if;
      ...
      --- Note: Container category does not add a join here; it will be appended once after terms are processed.
      when 'Containers' then
        hasContainerCriteria := 1;
    ...
    numOpenParens := c1_rec.openparens;
    numCloseParens := c1_rec.closeparens;
    ...
    ----------------------------------------------------------------------
    -- If this term is a container term, build it into varContainerAnchorConditional
    -- using alias root_container.<column>, otherwise build into varConditional as before.
    ----------------------------------------------------------------------
    if varSearchCategory = 'Containers' then
      -- Force column reference to the anchor alias that will exist in the container subtree join.
      varColumnName := 'root_container.' || lower(varColumnBaseName);

      if c1_rec.searchTerm = 'NULL' THEN
        varContainerAnchorConditional :=
          varContainerAnchorConditional || ' ' || varJoinTerm || ' ' ||
          lpad('(',numOpenParens,'(') || varColumnName || ' IS NULL' || lpad(')',numCloseParens,')') || ' ';
      elsif c1_rec.searchTerm = 'NOT NULL' THEN
        varContainerAnchorConditional :=
          varContainerAnchorConditional || ' ' || varJoinTerm || ' ' ||
          lpad('(',numOpenParens,'(') || varColumnName || ' IS NOT NULL' || lpad(')',numCloseParens,')') || ' ';
      else
        case varDataType
          when 'CLOB' THEN
            RAISE_APPLICATION_ERROR(-20100,'Search on fields with data type of CLOB not supported yet.',TRUE);
          when 'DATE' THEN
            RAISE_APPLICATION_ERROR(-20110,'Search on container fields with data type DATE not supported in container anchor predicate.',TRUE);
          when 'NUMBER' THEN
            if upper(c1_rec.comparator) = 'IN' then
              varContainerAnchorConditional :=
                varContainerAnchorConditional || ' ' || varJoinTerm || ' ' ||
                lpad('(',numOpenParens,'(') || varColumnName || ' IN (' || ':bnd' || x;
              FOR y in 1..regexp_count(c1_rec.searchTerm, ',') LOOP
                x := x + 1;
                varContainerAnchorConditional := varContainerAnchorConditional || ',:bnd' || x;
              END LOOP;
              varContainerAnchorConditional := varContainerAnchorConditional || ')' || lpad(')',numCloseParens,')') || ' ';
            else
              varContainerAnchorConditional :=
                varContainerAnchorConditional || ' ' || varJoinTerm || ' ' ||
                lpad('(',numOpenParens,'(') || varColumnName || ' ' || c1_rec.comparator || ' ' || ':bnd' || x  || lpad(')',numCloseParens,')') || ' ';
            end if;
          when 'CHAR' THEN
            if upper(c1_rec.comparator) = 'LIKE' then
              varContainerAnchorConditional :=
                varContainerAnchorConditional || ' ' || varJoinTerm ||
                lpad('(',numOpenParens,'(') || ' upper(' || varColumnName || ') ' || c1_rec.comparator ||
                ' upper(''%''||' || ':bnd' || x || '||''%'')' || lpad(')',numCloseParens,')') || ' ';
            elsif c1_rec.comparator = '=' then
              varContainerAnchorConditional :=
                varContainerAnchorConditional || ' ' || varJoinTerm  ||
                lpad('(',numOpenParens,'(') || ' upper(' || varColumnName || ') = upper(' || ':bnd' || x || ')'  ||
                lpad(')',numCloseParens,')') || ' ';
              ...
              varContainerAnchorConditional :=
                varContainerAnchorConditional || ' ' || varJoinTerm || ' ' ||
                lpad('(',numOpenParens,'(') || varColumnName || ' ' || c1_rec.comparator || ' ' || ':bnd' || x  ||
                lpad(')',numCloseParens,')') || ' ';
            end if;
          when 'VARCHAR2' THEN
            if upper(c1_rec.comparator) = 'LIKE' then
          ...
          when 'CTXKEYWORD' THEN
            varContainerAnchorConditional :=
              varContainerAnchorConditional || ' ' || varJoinTerm ||
              ' contains(' || varColumnName || ', :bnd' || x || ',' || containLabel ||') > 0';
            containLabel := containLabel+1;
        end case;
      end if;

    else
      ----------------------------------------------------------------------
      -- EXISTING behavior for all non-container terms: build into varConditional
      ----------------------------------------------------------------------
      if c1_rec.searchTerm = 'NULL' THEN
        varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' IS NULL' || lpad(')',numCloseParens,')') || ' ';
      elsif c1_rec.searchTerm = 'NOT NULL' THEN
        varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' IS NOT NULL' || lpad(')',numCloseParens,')') || ' ';
      else
        case varDataType
          when 'CLOB' THEN
            RAISE_APPLICATION_ERROR(-20100,'Search on fields with data type of CLOB not supported yet.',TRUE);
          when 'DATE' THEN
            if REGEXP_LIKE(c1_rec.searchTerm, '^[0-9]{4}-[0-9]{2}-[0-9]{2}$') THEN
              if c1_rec.comparator = '=' then
                varConditional := varConditional || ' ' || varJoinTerm  || lpad('(',numOpenParens,'(') || ' to_char(' || varColumnName || ',''yyyy-mm-dd'') = :bnd' || x  || lpad(')',numCloseParens,')') || ' ';
              elsif c1_rec.comparator = '>=' then
                varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' >= to_date( :bnd' || x || ', ''yyyy-mm-dd'')' || lpad(')',numCloseParens,')') || ' ';
              elsif c1_rec.comparator = '<=' then
                varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' <= to_date( :bnd' || x || ', ''yyyy-mm-dd'')' || lpad(')',numCloseParens,')') || ' ';
              else
              ...
              varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' BETWEEN to_date( :bnd' || x || ', ''yyyy-mm-dd'') and to_date( :bnda' || x || ', ''yyyy-mm-dd'')+INTERVAL ''1'' DAY - INTERVAL ''1'' SECOND'  || lpad(')',numCloseParens,')') || ' ';
          ...
          when 'NUMBER' THEN
            if upper(c1_rec.comparator) = 'IN' then
              varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' IN (' || ':bnd' || x;
              FOR y in 1..regexp_count(c1_rec.searchTerm, ',') LOOP
                x := x + 1;
                varConditional := varConditional || ',:bnd' || x;
              END LOOP;
              varConditional := varConditional || ')'  || lpad(')',numCloseParens,')') || ' ';
            else
              varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' ' || c1_rec.comparator || ' ' || ':bnd' || x  || lpad(')',numCloseParens,')') || ' ';
            end if;
          when 'CHAR' THEN
            if upper(c1_rec.comparator) = 'LIKE' then
              varConditional := varConditional || ' ' || varJoinTerm || lpad('(',numOpenParens,'(') || ' upper(' || varColumnName || ') ' || c1_rec.comparator || ' upper(''%''||' || ':bnd' || x || '||''%'')'  || lpad(')',numCloseParens,')') || ' ';
            elsif c1_rec.comparator = '=' then
              varConditional := varConditional || ' ' || varJoinTerm  || lpad('(',numOpenParens,'(') || ' upper(' || varColumnName || ') = upper(' || ':bnd' || x || ')'  || lpad(')',numCloseParens,')') || ' ';
            elsif upper(c1_rec.comparator) = 'IN' then
              varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' IN (' || ':bnd' || x ;
              FOR y in 1..regexp_count(c1_rec.searchTerm, ',') LOOP
                x := x + 1;
                varConditional := varConditional || ',:bnd' || x;
              END LOOP;
              varConditional := varConditional || ')'  || lpad(')',numCloseParens,')') || ' ' || lpad(')',numCloseParens,')') || ' ';
            else
              varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' ' || c1_rec.comparator || ' ' || ':bnd' || x  || lpad(')',numCloseParens,')') || ' ';
            end if;
          when 'VARCHAR2' THEN
            if upper(c1_rec.comparator) = 'LIKE' then
              ...
              varConditional := varConditional || ' ' || varJoinTerm || lpad('(',numOpenParens,'(') || ' soundex(' || varColumnName || ') = soundex(' || ':bnd' || x || ')'  || lpad(')',numCloseParens,')') || ' ';
            elsif upper(c1_rec.comparator) = 'JARO_WINKLER' then
              varConditional := varConditional || ' ' || varJoinTerm || lpad('(',numOpenParens,'(') || ' (utl_match.jaro_winkler_similarity(' || varColumnName || ', ' || ':bnd' || x || ') >= 80) '  || lpad(')',numCloseParens,')') || ' ';
            elsif upper(c1_rec.comparator) = 'IN' then
              varConditional := varConditional || ' ' || varJoinTerm || ' ' || lpad('(',numOpenParens,'(') || varColumnName || ' IN (' || ':bnd' || x;
              FOR y in 1..regexp_count(c1_rec.searchTerm, ',') LOOP
                x := x + 1;
          ...
          when 'CTXKEYWORD' THEN
            varConditional := varConditional || ' ' || varJoinTerm || ' contains(' || varColumnName || ', :bnd' || x || ',' || containLabel ||') > 0';
            containLabel := containLabel+1;
        end case;
      end if;
    end if;
  ...
  ----------------------------------------------------------------------
  -- Append the container subtree join iff container criteria exist
  ----------------------------------------------------------------------
  if hasContainerCriteria = 1 then
    if varContainerAnchorConditional is null then
      raise_application_error(-20120,'Container search terms present but no container anchor predicate was generated.',true);
    end if;

    varContainersJoin :=
      '
       ---containers: match anchor container(s), expand descendant subtree, then join to current containers of parts
       join specimen_part cont_part
         on (cataloged_item.collection_object_id = cont_part.derived_from_cat_item)
       join coll_obj_cont_hist cont_hist
         on (cont_part.collection_object_id = cont_hist.collection_object_id
             and cont_hist.current_container_fg = 1)
       join (
           select c.container_id
           from container c
           start with c.container_id in (
               select root_container.container_id
               from container root_container
               where 1=1 ' || varContainerAnchorConditional || '
           )
           connect by prior c.container_id = c.parent_container_id
       ) container_desc
         on (cont_hist.container_id = container_desc.container_id)
      ';

    varJOINS := varJOINS || varContainersJoin;
  end if;

  varSQL := varSQL || varJOINS || ' WHERE 1=1 ' || varConditional;

  populateTableSQL :=
    'insert into ' || varusername || '.user_search_table (result_id, collection_object_id, pagesort)
     select resultID, collection_object_id, rownum from (' || varSQL || ')';

  ---was the last search term a catalog number? If so...add a closing ')'
  if isCatNum then
    populateTableSQL := populateTableSQL || ')';
  end if;
