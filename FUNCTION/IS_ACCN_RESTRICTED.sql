  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "IS_ACCN_RESTRICTED" (COLLECTION_OBJECT_ID IN NUMBER) 
--  Given a collection_object_id, return 1 if there is a non-null value in the restrictions on use
...
--    value for the restrictions on use.
RETURN NUMBER AS 
   ...
   BEGIN
       ...
       open l_cur for '
         ...
         where cataloged_item.collection_object_id = :x
			...
			and length(trim(permit.restriction_summary)) > 0
       '
