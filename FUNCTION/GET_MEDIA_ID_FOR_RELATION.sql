  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "GET_MEDIA_ID_FOR_RELATION" 
(
  RELATED_PRIMARY_KEY IN NUMBER 
, RELATIONSHIP IN VARCHAR2 
, MIME_TYPE in VARCHAR2
) RETURN NUMBER 
--  Given a media relationship and a replated primary key, and mimetype, obtain the media id, or null if no media for that relationship type
...
--  @return a media id for the related media record with the provided mime type, or null if no match.
AS
