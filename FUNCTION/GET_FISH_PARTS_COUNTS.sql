  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "GET_FISH_PARTS_COUNTS" 
...
-- Dry specimen_parts that exist for that collection_object    --
-- in the form "x Alc, y C&S, z Dry", where x, y and z are either --
...
-- record], but expanded from Skel to Dry.  --
-- Changed from Skel to Dry on 2025 Sept 22, per Ichthyology dept. --
as
...
group by derived_from_cat_item
union
select 3 as ordinal, ''Dry'', sum(coll_object.lot_count)
from specimen_part  
left join coll_object on specimen_part.collection_object_id = coll_object.collection_object_id  
where preserve_method = ''dry''
...
group by derived_from_cat_item'
       ...
       close l_cur;
       l_str := l_alc_val || ' Alc, ' || l_cs_val || ' C&S, ' || l_skel_val || ' Dry';
