...
a_instn varchar2(255);
  BEGIN
        FOR rec IN (SELECT * FROM bulkloader_lepidoptera where collection_object_id=colobjid ) LOOP
                BEGIN
                thisError := '';
                ...
                IF rec.collecting_event_id IS NULL THEN
                   IF rec.locality_id IS NULL THEN -- only care about locality if no event picked                       
                        ...
                        execute immediate 'select 
                                ...
                                from bulkloader_lepidoptera where  collection_object_id = ' || rec.collection_object_id into 
                                    attributeType,
                                        attributeValue,
                                        attributeDate,
                                        attributeDeterminer;
                ...
                for i IN 1 .. 12 LOOP -- number of parts
                        ...
                        if partName is not null then
                                ...
                                /*if partBarcode is not null then
                                        ...
                                        if numRecs != 0 then
                                                thisError :=  thisError || '; barcode_' || i || ' is a label';
                                        END IF;
